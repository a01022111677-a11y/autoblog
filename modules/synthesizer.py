...
5. 핵심 요약 박스: 글 서두(첫 번째 이미지 밑쯤)에 가장 중요한 내용을 3줄로 요약하는 박스를 만듭니다. (마크다운 인용구 `>` 활용)
   예시:
   > 📌 **{keyword} 핵심 3줄 요약**
   > ♦ (첫 번째 핵심 내용)
   > ♦ (두 번째 핵심 내용)
   > ♦ (세 번째 핵심 내용)
6. 본문 내용 (가독성 극대화): 스마트폰으로 읽는 사람들을 위해 **문장을 짧게 끊어 쓰고, 두세 줄마다 반드시 엔터(줄바꿈)를 여러 번 쳐서 여백을 넉넉하게** 주세요. 소제목(##)을 활용하여 내용이 빽빽해 보이지 않게 하고 중간중간 제공된 이미지를 넣어 시선을 분산시켜주세요. 말투는 전문가처럼 딱딱하게 쓰지 말고 이웃에게 이야기하듯 아주 친근하고 호들갑스러운 '해요체'를 사용하세요.

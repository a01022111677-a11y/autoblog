...
5. 핵심 요약 박스 (예쁜 테두리 박스): 글 서두(첫 번째 이미지 밑쯤)에 가장 중요한 내용을 3줄로 요약하는 박스를 만듭니다. 다크모드에서도 예쁘게 보이도록 배경색은 빼고 테두리만 넣습니다.
   예시:
   <div style="border: 2px solid #ff4b4b; padding: 15px; border-radius: 10px; text-align: left; margin: 20px 0;">
   📌 <b>{keyword} 핵심 3줄 요약</b><br><br>
   ♦ (첫 번째 핵심 내용)<br>
   ♦ (두 번째 핵심 내용)<br>
   ♦ (세 번째 핵심 내용)
   </div>
6. 눈에 띄는 소제목: 밋밋한 `##` 대신, 소제목은 반드시 아래처럼 빨간 밑줄이 쳐지는 HTML 태그를 사용해서 시선을 사로잡게 만드세요.
   예시: `<h3 style="border-bottom: 2px solid #ff4b4b; padding-bottom: 5px; margin-top: 30px;">🚀 여기에 소제목 입력</h3>`
7. 본문 내용 (가독성 및 형광펜 효과): 
   - 스마트폰으로 읽는 사람들을 위해 **문장을 짧게 끊어 쓰고, 두세 줄마다 반드시 엔터(줄바꿈)를 여러 번 쳐서 여백을 넉넉하게** 주세요. 
   - **[중요]** 본문에서 가장 핵심이 되는 문장이나 단어에는 형광펜 효과를 주기 위해 반드시 아래 HTML 태그를 3~4번 이상 적극적으로 사용하세요! (다크모드에서도 글씨가 보이도록 반드시 color: #000000; 를 넣어야 합니다)
     예시: `<span style="background-color: #fffacd; color: #000000; font-weight: bold; padding: 2px 4px;">여기에 강조할 내용</span>`
